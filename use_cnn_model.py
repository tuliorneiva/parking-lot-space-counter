# use_cnn_model.py
# Script para usar o modelo CNN treinado para classificar vagas de estacionamento
...
def classify_spot(img, mask, model, target_size=(32, 32)):
    """
    Classifica uma vaga como vazia ou ocupada usando o modelo CNN.

    Args:
        img: Imagem do estacionamento
        mask: Máscara da vaga
        model: Modelo CNN
        target_size: Tamanho para redimensionar a imagem
    ...
    spot_resized = resize(spot_img, target_size, anti_aliasing=True)

    # Normalizar para [0, 1]
    if spot_resized.max() > 1.0:
        spot_resized = spot_resized / 255.0

    # Adicionar dimensão de lote
    spot_batch = np.expand_dims(spot_resized, axis=0)

    # Classificar
    prediction = model.predict(spot_batch)[0][0]

    # Retorna True se vazia (< 0.5), False se ocupada (>= 0.5)
...
def process_parking_lot(img_path, masks_dir, mapping_file, model, xml_path=None, output_dir=None, lot_name=None):
    ...
    cv2.putText(img_result, f'Vagas disponíveis: {empty_count} / {total_count}', 
                (20, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    y_pos += 30
...
def main():
    parser = argparse.ArgumentParser(description='Classificar vagas de estacionamento usando modelo CNN')

    # Argumentos obrigatórios
    parser.add_argument('--model', type=str, required=True, help='Caminho para o modelo CNN')
        ...
        for img_file in os.listdir(args.image_dir):
            if img_file.endswith(('.jpg', '.jpeg', '.png')):
                ...
                if args.xml_dir:
                    ...
                    xml_path_candidate = os.path.join(args.xml_dir, xml_file)
                    if os.path.exists(xml_path_candidate):
                        xml_path = xml_path_candidate

                # Processar o estacionamento
                spots_status, img_result, metrics = process_parking_lot(
                    img_path, args.masks, args.mapping, model, xml_path, args.output, args.lot_name
                )
                ...
                plt.imshow(cv2.cvtColor(img_result, cv2.COLOR_BGR2RGB))
                plt.axis('off')
                plt.title(f'Status das Vagas - {args.lot_name or "Estacionamento"} - {img_file}')
                plt.show()
